...
public:
    int magnificentSets(int n, vector<vector<int>>& edges) {
        ...
        // Divide nodes of the graph into m groups st: Each node in the graph
        // belongs to exactly one group, pair connected by [ai, bi], if ai
        // belongs to the group with index x, and bi belongs to the group with
        // index y, then |y - x| = 1. Return maximum number of groups (maximum
        // m) else Return -1 if impossible to group nodes
