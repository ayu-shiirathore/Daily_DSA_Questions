class Solution {
public:
bool checkIfCycle(int node, int parent, unordered_set<int>& visited,
                      unordered_map<int, vector<int>>& adjacencyList) {
        visited.insert(node);
        // go thru all neighbours of node
        for (auto it : adjacencyList[node]) {
            if (it == parent) {
                // parent node is the node from where we reached here so prevent
                // going back skip parent node to avoid backtracking
                continue;
            }
            // if neighbour already visited then cycle might exist:
            if (visited.count(it) ||
                checkIfCycle(it, node, visited, adjacencyList)) {
                // cycle detected
                return true;
            }
        }
    ...
    vector<int> findRedundantConnection(vector<vector<int>>& edges) {
        ...
        // n+1 nodes in graph:
        int n = edges.size();
        // Return edge that can be removed st resulting graph is a tree of n
        // nodes
